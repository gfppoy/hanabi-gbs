...
import common_utils
from transformer_embedding_r2d2 import get_model
...
class R2D2Agent(torch.jit.ScriptModule):
    ...
    def act(self, obs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        assert(priv_s.shape[1] == 838)
        assert(seq_len.shape[1] == 1)
        ...
        aoh = obs["aoh"].reshape(priv_s.shape[0], 80, 838).transpose(0, 1).to(self.device)
        #aoh = torch.zeros((80, priv_s.shape[0], 838), device=self.device)
        #for i in range(priv_s.shape[0]):
        #    for j in range(int(seq_len[i,0].item())):
        #        aoh[j, i, :] = obs["aoh"+str(j+1)][i,:].reshape(838)

        #aoh[seq_len-1, torch.arange(start=0, end=, :] = priv_s[:,:]
        src = self.belief_module.get_samples_one_player(aoh, seq_len.reshape(seq_len.shape[0]).to(torch.long), device=self.device)
       ...
       #     bs = obs["s"].size(0)
       #     seq_len = obs["s"].size(1)
       #     src, _  = self.belief_module.get_samples_one_player(torch.cat([obs["s"].transpose(0,1), torch.zeros((79, bs, 838), device=self.device)], dim=0), obs["own_hand"].transpose(0,1), torch.zeros((bs), dtype=torch.long, device=self.device) + seq_len, device=self.device)
       # else:
       #     src, _ = self.belief_module.get_samples_one_player(torch.cat([obs["s"], torch.zeros((80-obs["s"].size(0), obs["s"].size(1), 838), device=self.device)], dim=0), obs["own_hand"], torch.zeros((obs["s"].size(1)), dtype=torch.long, device=self.device) + obs["s"].size(0), device=self.device)
