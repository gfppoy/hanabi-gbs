...
float searchMoveGeneralizedBelief(
    const hle::HanabiState& state,
    hle::HanabiMove move,
    const std::vector<std::vector<hle::HanabiCardValue>>& hands,
    const std::vector<int> seeds,
...
//  std::vector<std::vector<HybridModel>> allPlayers(hands.size(), players);
  std::vector<std::vector<HybridModel>> allPlayers;
  std::vector<GameSimulator> games;

  std::vector<std::vector<torch::Tensor>> aohs;
  games.reserve(hands.size());
  for (size_t i = 0; i < hands.size(); ++i) {
    std::vector<SimHand> simHands{
        SimHand(myIdx, hands[i]),
    };
    allPlayers.push_back({players[0], players[player_each_rollout[i]]});
    games.emplace_back(state, simHands, seeds[i]);
    aohs.push_back(aoh);
  }
...
hle::HanabiMove SpartaActor::spartaSearch(
    const GameSimulator& env, hle::HanabiMove bpMove, int numSearch, float threshold, std::vector<int> cardCount, std::vector<torch::Tensor> aoh, torch::Tensor seq_len, std::vector<int> player_each_rollout, 
...
//  }
  for (auto move : legalMoves) {
    futMoveScores.push_back(std::async(
        std::launch::async, searchMoveGeneralizedBelief, state, move, simHands, seeds, index, players, aoh, seq_len, player_each_rollout));
  }
